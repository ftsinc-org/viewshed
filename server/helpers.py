...
class CoordSystem(object):
    ...
    def lnglat_to_pixel(cls, lnglat, zoom=ZOOM):
        ...
        y = (
            ...
            / math.pi
            * 2 ** zoom
            * (math.pi - math.log(math.tan(math.pi / 4.0 + lat / 2.0)))
        )
    ...
    def pixel_to_lnglat(cls, point, zoom=ZOOM):
        x, y = point
        lat = (
            4.0 * math.atan(math.exp(math.pi - y * math.pi / (128.0 * 2 ** zoom)))
            - math.pi
        ) / 2.0
    ...
    @classmethod
    def pixels_to_lnglat(cls, x, y, zoom=ZOOM):
        return cls.pixel_to_lnglat((x, y), zoom)

    @classmethod
    def meters_per_pixel(cls, lnglat, zoom=ZOOM):
        """meters per pixel in epsg3857"""
        return (math.cos(lnglat[1] * math.pi / 180.0) * 2.0 * math.pi * 6378137.0) / (
...
class Tile(object):
    def __init__(self, zoom, pixel, url_template):
        ...
        self.pixel = (
            pixel[0] // 256 * 256,
            pixel[1] // 256 * 256,
        )  # round pixel to top left corner
    ...
    async def get_data(self):
        if not self.data_retrieved:
            ...
            self.retrieved_data = await self.data
    ...
    async def _retrieve_data(self):
        self.res = await AsyncHTTPClient().fetch(self.url, raise_error=False)
...
class TileSampler(object):
    ...
    def __init__(
        self, zoom=ZOOM, url_template="http://127.0.0.1:8080/{z}/{x}/{y}.tiff"
    ):
        self.zoom = zoom
        self.url_template = url_template
        self._tiles = {}
    ...
    def _unique_rows(self, data):
        ...
        """
        _, idxs = np.unique(
            data.view(data.dtype.descr * data.shape[1]), return_index=True
        )
        return data[sorted(idxs), :]

    async def _sample_tile_pixels(self, tile_pixel, pixels):
        ...
        xs = pixels[:, 0]
        ys = pixels[:, 1]
        tile_data = await self.get_tile(tile_pixel).get_data()
        if len(tile_data) == 0:
            return None
        return tile_data[ys.astype(int), xs.astype(int)]  # numpy is row column
    ...
    async def sample_pixels(self, pixels):
        ...
        for tile_pixel in tile_pixels:
            self.get_tile(tile_pixel)
        # sample tiles
        data = []
        for tile_pixel in tile_pixels:
            d = await self._sample_tile_pixels(tile_pixel, pixels)
            data.append(d)
        data = [d for d in data if d is not None]
        if len(data) == 0:
            return (None, pixels)  # if no tiles were found return None
        return (np.concatenate(data), pixels)

    async def sample_line(self, pixel1, pixel2):
        ...
        return await self.sample_pixels(pixels)

    async def sample_pixel(self, pixel):
        return (await self.sample_pixels(np.array([pixel])))[0]
